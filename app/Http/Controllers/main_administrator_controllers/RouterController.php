...
class RouterController extends Controller
{
    ...
    {
        return view('main-administrator-layouts.pendataan-perangkat-jaringan.data-perangkat.router.index', [
            'title' => 'Halaman Pendataan Perangkat Router',
        ]);
    ...
    public function routerJson(){
        $router_id_device_type = 1;
        $routers = Device::with(['device_type', 'device_brand', 'team_unit', 'menu'])->select([
            'devices.id',
          ...
          ->where('devices.id_device_type', '=', $router_id_device_type)
          ->orderBy('devices.id', 'asc'); 
    
        // Debugging: Tampilkan nilai user_nip sebelum query dieksekusi
        return Datatables::of($routers)->make(true);
