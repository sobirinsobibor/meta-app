...
class APWifiController extends Controller
{
    /**
    ...
    {
        return view('main-administrator-layouts.pendataan-perangkat-jaringan.data-perangkat.ap_wifi.index', [
            'title' => 'Halaman Pendataan Perangkat AP Wifi',
        ]);
    ...
    public function APWifiJson(){
        $ap_wifi_id_device_type = 4;
        $ap_wifies = Device::with(['device_type', 'device_brand', 'team_unit', 'menu'])->select([
            'devices.id',
          ...
          ->where('devices.id_device_type', '=', $ap_wifi_id_device_type)
          ->orderBy('devices.id', 'asc'); 
    
        // Debugging: Tampilkan nilai user_nip sebelum query dieksekusi
        return Datatables::of($ap_wifies)->make(true);
    }
